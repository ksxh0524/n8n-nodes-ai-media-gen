		...
		properties: [
			{
				...
				type: 'options',
				required: true,
				options: ConfigManager.getNodeModels('Z-Image').map((model) => ({
					name: model.id,
					value: model.id,
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (addToConfig) {
					ConfigManager.addCustomModel('Z-Image', addToConfig);
				...
				this.logger?.info('Starting image generation', { model: actualModel, prompt });
