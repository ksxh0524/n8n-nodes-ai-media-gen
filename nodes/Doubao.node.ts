...
export class Doubao implements INodeType {
	description: {
		displayName: 'Doubao',
		name: 'doubao',
		...
		properties: [
			{
				...
				type: 'options',
				required: true,
				options: ConfigManager.getNodeModels('Doubao').map((model) => ({
					name: model.id,
					value: model.id,
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (addToConfig) {
					ConfigManager.addCustomModel('Doubao', addToConfig);
				...
				if (model === 'wanx-video-v1') {
					response = await QwenProvider.generateVideo({
						model: actualModel,
						prompt,
